/**
 * cg_rf_receiver_firewall_opa.cpp — Receiver's RF for OPA/PSI (persistent thread pool)
 *
 * KEY FIX: global_pool() creates worker threads exactly once at startup.
 * The first parallel_for warms up thread_local CG_Scheme on each worker (once).
 * All subsequent parallel_for calls pay zero construction overhead.
 */
...
// Force all workers to initialise their thread_local CG_Scheme before the
// online stream starts. This makes the later timed phases less noisy.
static void prewarm_pool(CGThreadPool& pool) {
    ...
    std::cerr << "[rf_receiver_opa] pool pre-warmed in " << ms << " ms\n";
}
...
int main() {
    ...
    prewarm_pool(pool);  // one-time cost — amortised over all OLEs
    ...
    std::cerr << "[rf_receiver_opa] listening on :" << port_rfr
    ...
    CGNet::send_pk(rfs_fds[0], pk_prime);
    ...
    std::cerr << "[rf_receiver_opa] [RANDOMIZE AND FORWARD] Generating randomizers"
    ...
    std::cerr << "[rf_receiver_opa] R1 " << n_pts << "/" << n_pts
              << " over " << LANES << " lane(s)\n";
    ...
    std::cerr << "[rf_receiver_opa] R2 " << n_pts << "/" << n_pts
              << " over " << LANES << " lane(s)\n";
