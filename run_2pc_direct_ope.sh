...
# Launch one direct OPE-over-OLE experiment and verify p(alpha) offline.

source "$(cd "$(dirname "$0")" && pwd)/scripts/2pc_common.sh"

N="${1:-1000}"
INPUT_BITS="${CG_INPUT_FILE_BITS:-128}"
init_run_dir "run_2pc_direct_ope_${N}"
...
echo "Preparing random ${INPUT_BITS}-bit OPE input files outside timed protocol..."
...
run_pair "direct_ope_${N}" "local_first" \
    "cd '$LOCAL_ROOT/$BUILD_DIR' && $ENV_LOCAL CG_BENCH_IO_DIR='$LOCAL_IO' CG_PROTOCOL_TIMING_FILE='$LOCAL_IO/protocol_time.csv' ./cg_ope_receiver '$N' --alpha-file '$LOCAL_IO/alpha.txt' 9003" \
    "cd '$REMOTE_ROOT/$BUILD_DIR' && $ENV_REMOTE CG_BENCH_IO_DIR='$REMOTE_IO' CG_PROTOCOL_TIMING_FILE='$REMOTE_IO/protocol_time.csv' ./cg_ope_sender '$LOCAL_IP' '$N' --input-file '$REMOTE_IO/coeffs.txt' 9003"
