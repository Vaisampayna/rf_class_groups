...
/*
 * Direct three-round OLE sender.
 *
 * Generates the AHE key, encrypts a and b, decrypts the receiver's masked
 * ciphertext result, and returns the plaintext z to the receiver.
 */
...
int main(int argc, char** argv)
{
    ...
    const char* port = (argc > 3) ? argv[3] : ole3_port_rec();
    ...
    std::cerr << "[ole3_sender] benchmark input sampling excluded from protocol time: "
              << ms_since(t_input) << " ms\n";
    ...
    std::cerr << "[ole3_sender] key generation done in "
    ...
    write_ole_sender_inputs("ole3_sender_inputs.txt", a_vals, b_vals);
    close_rf_lanes(fds);
    std::cerr << "[ole3_sender] protocol end-to-end excluding input sampling done in "
              << protocol_ms << " ms\n";
    (void)cs;
    return 0;
}
