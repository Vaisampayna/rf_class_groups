...
run_pair "direct_psi_${N}" "local_first" \
    "rm -rf '$LOCAL_IO'; mkdir -p '$LOCAL_IO'; cd '$LOCAL_ROOT'; python3 generate_sets.py '$N' '$N' '$OVERLAP' '$LOCAL_IO/set_A.txt' '$LOCAL_IO/set_B.txt' '$LOCAL_IO/true_intersection.txt' '$SEED' '$PSI_BITS'; cd '$LOCAL_ROOT/$BUILD_DIR' && export $ENV_LOCAL CG_PROTOCOL_TIMING_FILE='$LOCAL_IO/protocol_time.csv'; ./cg_psi_receiver '$N' --input-file '$LOCAL_IO/set_B.txt' --output-file '$LOCAL_IO/intersection_out.txt' --timing-file '$LOCAL_IO/protocol_time.csv' --port 9103" \
    "rm -rf '$REMOTE_IO'; mkdir -p '$REMOTE_IO'; cd '$REMOTE_ROOT'; python3 generate_sets.py '$N' '$N' '$OVERLAP' '$REMOTE_IO/set_A.txt' '$REMOTE_IO/set_B.txt' '$REMOTE_IO/true_intersection.txt' '$SEED' '$PSI_BITS'; cd '$REMOTE_ROOT/$BUILD_DIR' && export $ENV_REMOTE CG_PROTOCOL_TIMING_FILE='$REMOTE_IO/protocol_time.csv'; ./cg_psi_sender '$LOCAL_IP' '$N' --input-file '$REMOTE_IO/set_A.txt' --port 9103"
