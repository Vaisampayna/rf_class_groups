...
/*
 * Reverse-firewalled three-round OLE receiver.
 *
 * Executes the three-round OLE receiver role through the RF-OLE3 firewalls.
 * The receiver clock stops when final OLE outputs are in memory.
 */
...
int main(int argc, char** argv)
{
    ...
    std::cerr << "[rf_ole3_receiver] benchmark input sampling excluded from protocol time: "
              << ms_since(t_input) << " ms\n";

    // Receiver-owned protocol clock for RF 3-round OLE: includes the firewall
    // network path and stops once final y_i outputs are in memory.
    ...
    std::cerr << "[rf_ole3_receiver] listening on :" << port
              << " for " << LANES << " lane(s)\n";
    ...
    write_protocol_timing_file_from_env("rf_ole3_receiver", protocol_ms);

    write_ole_receiver_io("rf_ole3_receiver_io.txt", q, x_vals, out_vals);
    close_rf_lanes(fds);
    std::cerr << "[rf_ole3_receiver] protocol end-to-end excluding input sampling done in "
              << protocol_ms << " ms\n";
    return 0;
}
